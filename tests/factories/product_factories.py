...
from store.models import Category, Product, ProductReview, ProductTag
...
class ProductFactory(DjangoModelFactory):
    ...
    dimensions_length = factory.Faker(
        "pydecimal", left_digits=2, right_digits=1, positive=True
    )
    dimensions_width = factory.Faker(
        "pydecimal", left_digits=2, right_digits=1, positive=True
    )
    dimensions_height = factory.Faker(
        "pydecimal", left_digits=2, right_digits=1, positive=True
    )
    ...
    meta_description = factory.LazyAttribute(
        lambda obj: f"Buy {obj.name} online. {obj.short_description}"
    )
    meta_keywords = factory.LazyAttribute(
        lambda obj: f"{obj.name}, {obj.category.name}, online shopping"
    )
