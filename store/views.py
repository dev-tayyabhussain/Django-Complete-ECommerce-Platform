...
from .models import Product, Category, ProductTag, ProductReview, Wishlist
...
class ProductListView(ListView):
    ...
    def get_queryset(self):
        ...
        if search_query:
            ...
            logger.info(f"Search query '{search_query}' returned {queryset.count()} products")
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add popular tags
        context['popular_tags'] = ProductTag.objects.filter(
            is_active=True
        ).annotate(
            product_count=Count('products', filter=Q(products__is_active=True))
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if product.tags.exists():
            context['tagged_products'] = Product.objects.filter(
...
class TagDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add related tags
        context['related_tags'] = ProductTag.objects.filter(
            is_active=True
        ).exclude(id=tag.id).annotate(
            common_products=Count('products', filter=Q(products__tags=tag))
        ).filter(common_products__gt=0).order_by('-common_products')[:5]
