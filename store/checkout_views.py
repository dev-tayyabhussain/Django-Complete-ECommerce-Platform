...
from .models import (
    Cart, CartItem, Order, OrderItem, Address, PaymentMethod, 
    UserProfile, Product
)
from .forms import CheckoutForm, AddressForm, PaymentMethodForm
...
class CheckoutView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        coupon_discount = getattr(self.request, 'session', {}).get('coupon_discount', {})
        discount_amount = Decimal('0.00')
        
        if applied_coupon and coupon_discount:
            if coupon_discount.get('type') == 'percentage':
                discount_amount = subtotal * Decimal(str(coupon_discount.get('discount', 0)))
            else:
    ...
    def form_valid(self, form):
        ...
        try:
            with transaction.atomic():
                ...
                # Get addresses
                use_billing_for_shipping = form.cleaned_data.get('use_billing_for_shipping', True)
                billing_address = form.cleaned_data['billing_address']
                shipping_address = billing_address if use_billing_for_shipping else form.cleaned_data['shipping_address']
                ...
                logger.info(f"Order created: {order.order_number} for user {self.request.user.username}")
        ...
        try:
            ...
            use_billing_for_shipping = form.cleaned_data.get('use_billing_for_shipping', False)
...
class OrderConfirmationView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'breadcrumbs': [
                {'name': 'Home', 'url': '/'},
                {'name': 'Order Confirmation', 'url': reverse('store:order_confirmation')}
            ]
        })
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'breadcrumbs': [
                {'name': 'Home', 'url': '/'},
                {'name': 'My Orders', 'url': reverse('store:order_list')},
                {'name': f'Order {order.order_number}', 'url': order.get_absolute_url()}
            ]
        })
...
def add_tracking_number(request, order_id):
    ...
    try:
        ...
        logger.info(f"Tracking number {tracking_number} added to order {order.order_number}")
