...
from .models import (
    ProductReview, Product, Category, ProductTag,
    UserProfile, Address, PaymentMethod, Cart, CartItem
)
...
class AdvancedSearchForm(ProductSearchForm):
    ...
    is_featured = forms.BooleanField(
        ...
        widget=forms.CheckboxInput(attrs={
            ...
            'aria-label': 'Featured products only'
        })
    )
...
class WishlistForm(forms.Form):
    ...
    def clean_product_id(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                _('Product not found or no longer available.')
            )
...
class PaymentMethodForm(forms.ModelForm):
    ...
    class Meta:
        model = PaymentMethod
        fields = ['payment_type', 'card_brand', 'card_last_four', 'expiry_month', 'expiry_year', 'is_default']
        widgets = {
            'payment_type': forms.Select(attrs={
                'class': 'form-select',
                'aria-label': 'Payment type'
            }),
            'card_brand': forms.TextInput(attrs={
                'class': 'form-control',
            ...
            'expiry_month': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'MM',
                'maxlength': '2',
                'aria-label': 'Expiry month'
            }),
            'expiry_year': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'YYYY',
                'maxlength': '4',
    ...
    def clean_card_last_four(self):
        ...
        if card_last_four:
            if not card_last_four.isdigit():
                raise forms.ValidationError("Card last four digits must contain only numbers.")
            if len(card_last_four) != 4:
                raise forms.ValidationError("Card last four digits must be exactly 4 digits.")
    ...
    def clean_expiry_year(self):
        ...
        if expiry_year:
            ...
            if year < current_year or year > current_year + 20:
                raise forms.ValidationError(f"Expiry year must be between {current_year} and {current_year + 20}.")
        return expiry_year
...
class CheckoutForm(forms.Form):
    ...
    def clean(self):
        ...
        if use_billing_for_shipping and not billing_address:
            raise forms.ValidationError("Please select a billing address to use for shipping.")
