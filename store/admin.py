...
from .models import (
    Category, Product, ProductImage, ProductTag, ProductReview, Wishlist,
    UserProfile, Address, PaymentMethod, Cart, CartItem, Order, OrderItem
)
...
class ProductAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'is_in_stock', 'is_active', 'is_featured', 'view_count', 'created_at'
    ]
    list_filter = [
        'is_active', 'is_featured', 'is_bestseller', 'is_in_stock',
...
class ProductTagAdmin(admin.ModelAdmin):
    ...
    list_display = ['name', 'slug', 'color_preview', 'product_count', 'is_active', 'created_at']
    list_filter = ['is_active', 'created_at']
    search_fields = ['name', 'description']
    prepopulated_fields = {'slug': ('name',)}
    ...
    def product_count(self, obj):
        ...
        count = obj.products.count()
        if count > 0:
            url = reverse('admin:store_product_changelist') + f'?tags__id__exact={obj.id}'
            return format_html('<a href="{}">{} products</a>', url, count)
        return '0 products'
    product_count.short_description = 'Products'
...
class AddressAdmin(admin.ModelAdmin):
    ...
    list_display = ['user', 'first_name', 'last_name', 'city', 'state', 'address_type', 'is_default', 'created_at']
    list_filter = ['address_type', 'is_default', 'country', 'created_at']
    search_fields = ['user__username', 'first_name', 'last_name', 'city', 'state', 'postal_code']
    readonly_fields = ['created_at', 'updated_at']
    ordering = ['-created_at']
    list_per_page = 25
...
class CartAdmin(admin.ModelAdmin):
    ...
    list_display = ['user', 'session_key', 'item_count', 'total_price', 'created_at', 'updated_at']
...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        'order_number', 'created_at', 'updated_at', 'subtotal', 
